		...
		eval: async function (posts_id, status, pfx, options) {
			...
			`; console.debug(status); //for development purposes, log each status
				...
				if (status.media_attachments.length > 0) {
					if (status.media_attachments[0].type == "image") {
				...
				<img src="${status.media_attachments[0].preview_url}" />
						...
						document.getElementById(pfx + "-postcard-" + status.id + "-media").addEventListener("click", function () { vsub.gallery(status.media_attachments) });
					} else if (status.media_attachments[0].type == "video") {
						let ze = function () {
							let a = document.createElement("a");
							a.addEventListener("click", function () { vsub.gallery(status.media_attachments); });
							return a.toString()
						};
						document.getElementById(pfx + "-postcard-" + status.id + "-content").outerHTML += `<div class="mdl-card__media" id="${pfx}-postcard-${status.id}-media">
...
Object.defineProperty(v, 'help', { get: function () { console.log("There is no help for you."); return null; }, set: function () { console.error("You can't set that!") } });
let vi = {
	feed_type: "home",
	feed_local: false,
	tmp_val: null,
...
let vsub = {
	...
	status: async function () {
		...
		api.get("statuses/" + zxhash).then(function (xstatus) {
			...
			v.status.eval("STATFOCUS",xstatus,"focus-status",{focused:true});
		});
		await v.status.getAll("SUBSTATUS", zxhash, api);
	},
	gallery: function (media_attachments) {
		let zsf;
		if ($("#tab-id-home.is-active").length == 1) zsf = "#POSTS";
		else if ($("#tab-id-profile.is-active").length == 1) zsf = "#MYPOSTS";
		else throw new Error("No real tab active");
		let gals = [];
		for (var ez of media_attachments) {
			if (ez.type == "image") {
				gals.push({ src: ez.url, msrc: ez.preview_url });
			} else if (ez.type == "video") {
				gals.push({ html: '<video autoplay="false" controls="true" src="' + ez.url + '">Video not supported</video>' })
			} // gifv/unknown not supported
		}
		//todo: add gallery
	},
